...
#ifdef NINTENDO_WII
	if(texture == nil){
		static uint32 nullTextureCount;
		if(nullTextureCount < 8)
			wiiLog("WII RenderWare: RwTextureGetRaster called with null texture caller=%p\n",
			       __builtin_return_address(0));
		nullTextureCount++;
		if(nullTextureCount == 8)
			wiiLog("WII RenderWare: further null texture messages suppressed\n");
		return nil;
	}
#endif
