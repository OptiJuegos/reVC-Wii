...
// Wii performance profile: keep the memory/per-frame reductions from the
// constrained-console path while retaining Wii/GX visual correctness.
#ifdef NINTENDO_WII
#define WII_PS2_SQUEEZE
#endif
...
#ifdef WII_PS2_SQUEEZE
	MODELINFOSIZE = 4900,	// PS2 value documented by reVC
#else
	MODELINFOSIZE = 6500,
#endif
	TXDSTORESIZE = 1385,
...
#ifdef NINTENDO_WII
// GX has no camera-texture copy path for CPostFX yet.  Enabling the extended
// filter makes its missing back buffer render as an untextured white quad.
...
#ifdef NINTENDO_WII
// ASND exposes stereo PCM voices rather than a 3D/reverb engine.  Let the
// game mixer calculate attenuation and pan before samples reach the backend.
#undef EXTERNAL_3D_SOUND
#undef AUDIO_REFLECTIONS
#undef AUDIO_REVERB
#undef AUDIO_CACHE
#undef AUDIO_OAL_USE_MPG123
...
// SQUEEZE_PERFORMANCE spreads/avoids expensive work in several original
// systems (population, zones, projectiles, rubbish, roadblocks, etc.).
#ifdef WII_PS2_SQUEEZE
	#define SQUEEZE_PERFORMANCE
#endif
#ifdef SQUEEZE_PERFORMANCE
	// Keep the PS2 alpha-test emulation on GX. The display-list path supports
	// both alpha passes and removing it changes fences/foliage/vehicle visuals.
	#ifndef NINTENDO_WII
