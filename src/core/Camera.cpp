...
{
	if(m_bFading){
		float previousFade = m_fFLOATingFade;
...
#ifdef NINTENDO_WII
		if(!m_bFading)
			wiiLog("WII fade: complete direction=%d timeout=%.3f timestep=%.3f value=%.3f previous=%.3f time=%u\n",
			       m_iFadingDirection, m_fTimeToFadeOut, CTimer::GetTimeStepInSeconds(),
			       m_fFLOATingFade, previousFade, CTimer::GetTimeInMilliseconds());
#endif
	}
}
...
{
#ifdef NINTENDO_WII
	wiiLog("WII fade: request timeout=%.3f direction=%d current=%.3f active=%s timestep=%.3f time=%u\n",
	       timeout, direction, m_fFLOATingFade, m_bFading ? "yes" : "no",
	       CTimer::GetTimeStepInSeconds(), CTimer::GetTimeInMilliseconds());
#endif
	m_bFading = true;
