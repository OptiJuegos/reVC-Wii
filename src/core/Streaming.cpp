...
#define MB (1024*1024)
	...
	// A fixed budget, because both halves of the PC calculation below are wrong
	// on this console.
	//
	// Its floor raises anything under 65MB up to 65MB, on the reasoning that no
	// desktop has less than that to spare.  A Wii has 88MB in total -- 24MB of
	// MEM1, most of it already spoken for by the executable, the two framebuffers
	// and the GX FIFO, plus 64MB of MEM2 that MALLOC_MEM2 makes the whole heap --
	// so a 65MB budget is never reached, nothing is ever evicted, and the heap is
	// exhausted while streaming still believes it has room.  That is the crash
	// after a few minutes of driving: the texture memory in librw's GX frame
	// trace climbs steadily and never comes back down.
	//
	// And the vehicle count underflows.  ms_memoryAvailable is a size_t, so the
...
#ifdef NINTENDO_WII
	static uint32 wiiUpdateCount;
	bool traceWiiUpdate = wiiUpdateCount++ < 4;
	if(traceWiiUpdate)
		wiiLog("WII streaming: update begin frame=%u requested=%d\n",
		       wiiUpdateCount, ms_numModelsRequested);
#endif
...
#ifdef NINTENDO_WII
	if(traceWiiUpdate)
		wiiLog("WII streaming: update complete frame=%u requested=%d\n",
		       wiiUpdateCount, ms_numModelsRequested);
#endif
}
...
CStreaming::LoadCdDirectory(const char *dirname, int n)
{
	...
	while(CFileMgr::Read(fd, (char*)&direntry, sizeof(direntry))){
		direntry.ConvertFromLittleEndian();
...
CStreaming::ConvertBufferToObject(int8 *buf, int32 streamId)
{
	...
	if(streamId < STREAM_OFFSET_TXD){
...
#ifdef NINTENDO_WII
		if(streamId == MI_MALE01)
			wiiLog("WII streaming: male01 conversion begin sectors=%d txd=%d anim=%d\n",
			       cdsize, mi->GetTxdSlot(), mi->GetAnimFileIndex());
#endif
...
#ifdef NINTENDO_WII
			if(streamId == MI_MALE01)
				wiiLog("WII streaming: male01 DFF conversion failed\n");
#endif
...
#ifdef NINTENDO_WII
		if(streamId == MI_MALE01)
			wiiLog("WII streaming: male01 conversion complete object=%p\n", mi->GetRwObject());
#endif
	}else if(streamId >= STREAM_OFFSET_TXD && streamId < STREAM_OFFSET_COL){
		// Txd
...
#ifdef NINTENDO_WII
			wiiLog("WII streaming: TXD id=%d rejected flags=0x%x no requested users\n",
			       streamId - STREAM_OFFSET_TXD, ms_aInfoForModel[streamId].m_flags);
#endif
			RemoveModel(streamId);
			RwStreamClose(stream, &mem);
			return false;
		...
		if(!success){
#ifdef NINTENDO_WII
			wiiLog("WII streaming: TXD id=%d conversion failed sectors=%d\n",
			       streamId - STREAM_OFFSET_TXD, cdsize);
#endif
...
#ifdef NINTENDO_WII
		if(streamId - STREAM_OFFSET_TXD == 11)
			wiiLog("WII streaming: TXD id=11 conversion complete dictionary=%p\n",
			       CTxdStore::GetSlot(11)->texDict);
#endif
	}else if(streamId >= STREAM_OFFSET_COL && streamId < STREAM_OFFSET_ANIM){
...
#ifdef NINTENDO_WII
		wiiLog("WII streaming: animation id=%d parse begin sectors=%d\n",
		       streamId - STREAM_OFFSET_ANIM, cdsize);
#endif
		CAnimManager::LoadAnimFile(stream, true, nil);
#ifdef NINTENDO_WII
		wiiLog("WII streaming: animation id=%d associations begin\n",
		       streamId - STREAM_OFFSET_ANIM);
#endif
		CAnimManager::CreateAnimAssocGroups();
#ifdef NINTENDO_WII
		wiiLog("WII streaming: animation id=%d conversion complete\n",
		       streamId - STREAM_OFFSET_ANIM);
#endif
		POP_MEMID();
	}
...
CStreaming::FinishLoadingLargeFile(int8 *buf, int32 streamId)
{
...
#ifdef NINTENDO_WII
		wiiLog("WII vehicle finalize: model=%d clump result=%s\n",
		       streamId, success ? "success" : "failure");
#endif
		if(success){
#ifdef USE_CUSTOM_ALLOCATOR
			RpClumpForAllAtomics((RpClump*)mi->GetRwObject(), RegisterAtomicMemPtrsCB, nil);
#endif
#ifdef NINTENDO_WII
			wiiLog("WII vehicle finalize: model=%d loaded-list begin count=%d last=%d\n",
			       streamId, ms_numVehiclesLoaded, ms_lastVehicleDeleted);
#endif
			success = AddToLoadedVehiclesList(streamId);
#ifdef NINTENDO_WII
			wiiLog("WII vehicle finalize: model=%d loaded-list result=%s count=%d last=%d\n",
			       streamId, success ? "success" : "failure",
			       ms_numVehiclesLoaded, ms_lastVehicleDeleted);
#endif
		}
#ifdef NINTENDO_WII
		wiiLog("WII vehicle finalize: model=%d cleanup POP_MEMID begin\n", streamId);
#endif
		POP_MEMID();
#ifdef NINTENDO_WII
		wiiLog("WII vehicle finalize: model=%d cleanup model ref begin refs=%d\n",
		       streamId, mi->GetNumRefs());
#endif
		mi->RemoveRef();
#ifdef NINTENDO_WII
		wiiLog("WII vehicle finalize: model=%d cleanup TXD ref begin slot=%d\n",
		       streamId, mi->GetTxdSlot());
#endif
		CTxdStore::RemoveRefWithoutDelete(mi->GetTxdSlot());
#if GTA_VERSION > GTAVC_PS2
		{
			int32 animIndex = mi->GetAnimFileIndex();
#ifdef NINTENDO_WII
			wiiLog("WII vehicle finalize: model=%d cleanup anim ref begin index=%d\n",
			       streamId, animIndex);
#endif
			if(animIndex != -1)
				CAnimManager::RemoveAnimBlockRefWithoutDelete(animIndex);
		}
#endif
	#ifdef NINTENDO_WII
		wiiLog("WII vehicle finalize: model=%d cleanup references complete\n", streamId);
	#endif
	}else if(streamId >= STREAM_OFFSET_TXD && streamId < STREAM_OFFSET_COL){
		// Txd
...
#ifdef NINTENDO_WII
	wiiLog("WII stream finalize: item=%d close begin success=%s\n",
	       streamId, success ? "yes" : "no");
#endif
	RwStreamClose(stream, &mem);
#ifdef NINTENDO_WII
	wiiLog("WII stream finalize: item=%d close complete\n", streamId);
#endif
	...
	UpdateMemoryUsed();
#ifdef NINTENDO_WII
	wiiLog("WII stream finalize: item=%d complete\n", streamId);
#endif
...
found:
	...
	if (pVehicleInfo->m_vehicleClass != -1){
#ifdef NINTENDO_WII
		wiiLog("WII vehicle finalize: model=%d car-array begin class=%d frequency=%d\n",
...
#ifdef NINTENDO_WII
	wiiLog("WII vehicle finalize: model=%d car-array complete\n", modelId);
#endif
	return true;
}
...
#ifdef NINTENDO_WII
	static uint32 wiiLoadAllCount;
	uint32 wiiLoadAllCall = ++wiiLoadAllCount;
	bool traceWiiLoadAll = wiiLoadAllCall <= 12;
	if(traceWiiLoadAll)
		wiiLog("WII streaming: load-all begin call=%u priority=%s requested=%d budget=%d\n",
		       wiiLoadAllCall, priority ? "yes" : "no", ms_numModelsRequested, numRequests);
#endif
...
#ifdef NINTENDO_WII
		if(streamId == 7050)
			wiiLog("WII streaming: load-all call=%u item=%d name=%s state=%d remaining=%d\n",
			       wiiLoadAllCall, streamId, GetObjectName(streamId),
			       ms_aInfoForModel[streamId].m_loadState, numRequests);
#endif
...
#ifdef NINTENDO_WII
			if(streamId == 7050)
				wiiLog("WII streaming: load-all call=%u converted item=%d result=%s state=%d\n",
				       wiiLoadAllCall, streamId, converted ? "yes" : "no",
				       ms_aInfoForModel[streamId].m_loadState);
			if(!converted){
				if(streamId < STREAM_OFFSET_TXD){
					CBaseModelInfo *failedModel = CModelInfo::GetModelInfo(streamId);
					wiiLog("WII streaming: load-all deferred model=%d name=%s txd=%d txdLoaded=%s anim=%d\n",
					       streamId, failedModel->GetModelName(), failedModel->GetTxdSlot(),
					       HasTxdLoaded(failedModel->GetTxdSlot()) ? "yes" : "no",
					       failedModel->GetAnimFileIndex());
				}else{
					wiiLog("WII streaming: load-all deferred item=%d after conversion failure\n", streamId);
				}
				break;
			}
#endif
			if(ms_aInfoForModel[streamId].m_loadState == STREAMSTATE_STARTED){
				bool finished = FinishLoadingLargeFile(ms_pStreamingBuffer[0], streamId);
#ifdef NINTENDO_WII
				if(streamId == 7050)
					wiiLog("WII streaming: load-all call=%u finish item=%d result=%s state=%d\n",
					       wiiLoadAllCall, streamId, finished ? "yes" : "no",
					       ms_aInfoForModel[streamId].m_loadState);
#endif
			}
...
#ifdef NINTENDO_WII
	if(traceWiiLoadAll)
		wiiLog("WII streaming: load-all complete call=%u requested=%d\n",
		       wiiLoadAllCall, ms_numModelsRequested);
#endif
}
#endif
...
#ifdef NINTENDO_WII
bool
CStreaming::LoadModelSynchronously(int32 model)
{
	uint32 position, size;
	int32 imageOffset;
	CStreamingInfo &info = ms_aInfoForModel[model];
	if(!info.GetCdPosnAndSize(position, size) || size > (uint32)ms_streamingBufferSize){
		wiiLog("WII streaming: synchronous model=%d rejected offset=%u size=%u capacity=%d\n",
		       model, position, size, ms_streamingBufferSize);
		return false;
	}

	if(info.m_loadState == STREAMSTATE_INQUEUE && info.m_next){
		DecrementRef(model);
		info.RemoveFromList();
	}

	imageOffset = GetCdImageOffset(CdStreamGetLastPosn());
	wiiLog("WII streaming: synchronous model=%d offset=%u imageOffset=%d size=%u\n",
	       model, position, imageOffset, size);
	if(CdStreamRead(0, ms_pStreamingBuffer[0], imageOffset + position, size) != STREAM_SUCCESS)
		return false;
	if(CdStreamSync(0) != STREAM_NONE)
		return false;

	const uint8 *header = (const uint8*)ms_pStreamingBuffer[0];
	wiiLog("WII streaming: synchronous model=%d header=%02x%02x%02x%02x %02x%02x%02x%02x %02x%02x%02x%02x\n",
	       model,
	       header[0], header[1], header[2], header[3],
	       header[4], header[5], header[6], header[7],
	       header[8], header[9], header[10], header[11]);

	info.m_loadState = STREAMSTATE_READING;
	MakeSpaceFor(size * CDSTREAM_SECTOR_SIZE);
	bool success = ConvertBufferToObject(ms_pStreamingBuffer[0], model);
	wiiLog("WII streaming: synchronous model=%d result=%s state=%d\n",
	       model, success ? "success" : "failure", info.m_loadState);
	return success;
}
#endif
