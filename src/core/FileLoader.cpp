...
CFileLoader::LoadLevel(const char *filename)
{
	...
	if(fd <= 0){
#ifdef NINTENDO_WII
		wiiLog("WII level file: cannot open %s\n", filename);
#endif
		RwTexDictionarySetCurrent(savedTxd);
		return;
	}
...
#ifdef NINTENDO_WII
	wiiLog("WII level file: opened %s fd=%d\n", filename, fd);
#endif
...
#ifdef NINTENDO_WII
			wiiLog("WII level file: COL begin %s\n", line + 10);
#endif
			LoadingScreenLoadingFile(line+10);
			LoadCollisionFile(line+10, 0);
#ifdef NINTENDO_WII
			wiiLog("WII level file: COL end %s\n", line + 10);
#endif
...
#ifdef NINTENDO_WII
			wiiLog("WII level file: IDE begin %s\n", line + 4);
#endif
			LoadingScreenLoadingFile(line + 4);
			LoadObjectTypes(line + 4);
#ifdef NINTENDO_WII
			wiiLog("WII level file: IDE end %s\n", line + 4);
#endif
		}else if(strncmp(line, "IPL", 3) == 0){
#ifdef NINTENDO_WII
			wiiLog("WII level file: IPL begin %s\n", line + 4);
#endif
			if(!objectsLoaded){
				LoadingScreenLoadingFile("Collision");
#ifdef NINTENDO_WII
				wiiLog("WII level IPL setup: object data begin\n");
#endif
				PUSH_MEMID(MEMID_WORLD);
				CObjectData::Initialise("DATA\\OBJECT.DAT");
#ifdef NINTENDO_WII
				wiiLog("WII level IPL setup: streaming begin\n");
#endif
				CStreaming::Init();
				POP_MEMID();
#ifdef NINTENDO_WII
				wiiLog("WII level IPL setup: collision store begin\n");
#endif
				PUSH_MEMID(MEMID_COLLISION);
				CColStore::LoadAllCollision();
				POP_MEMID();
#ifdef NINTENDO_WII
				wiiLog("WII level IPL setup: animation indices begin\n");
#endif
				...
				objectsLoaded = true;
#ifdef NINTENDO_WII
				wiiLog("WII level IPL setup: complete\n");
#endif
			}
			PUSH_MEMID(MEMID_WORLD);
			LoadingScreenLoadingFile(line + 4);
#ifdef NINTENDO_WII
			wiiLog("WII level IPL scene: parser begin\n");
#endif
			LoadScene(line + 4);
			POP_MEMID();
#ifdef NINTENDO_WII
			wiiLog("WII level file: IPL end %s\n", line + 4);
#endif
...
#ifdef NINTENDO_WII
	wiiLog("WII level file: parsed %s\n", filename);
#endif
...
static uint16
ReadCollisionUInt16(const void *source)
{
	uint16 value;
...
static uint32
ReadCollisionUInt32(const void *source)
{
	uint32 value;
	memcpy(&value, source, sizeof(value));
...
static float
ReadCollisionFloat(const void *source)
{
	float value;
	memcpy(&value, source, sizeof(value));
	return BSWAP_F32(value);
}

static CVector
ReadCollisionVector(const void *source)
{
	const uint8 *bytes = (const uint8*)source;
	return CVector(ReadCollisionFloat(bytes), ReadCollisionFloat(bytes+4), ReadCollisionFloat(bytes+8));
}
...
CFileLoader::LoadCollisionFile(const char *filename, uint8 colSlot)
{
	...
	while(CFileMgr::Read(fd, (char*)&header, sizeof(header))){
		uint32 headerIdent = ReadCollisionUInt32(&header.ident);
		uint32 headerSize = ReadCollisionUInt32(&header.size);
		assert(headerIdent == 'LLOC');
		if(headerIdent != 'LLOC' || headerSize < 24 || headerSize > sizeof(work_buff))
			break;
		CFileMgr::Read(fd, (char*)work_buff, headerSize);
...
CFileLoader::LoadCollisionFileFirstTime(uint8 *buffer, uint32 size, uint8 colSlot)
{
	...
	while(size > 8){
		...
		modelsize = ReadCollisionUInt32(&header->size);
		if(ReadCollisionUInt32(&header->ident) != 'LLOC')
			return size-8 < CDSTREAM_SECTOR_SIZE;
		if(modelsize < 24 || modelsize > size-8 || modelsize-24 > sizeof(work_buff))
			return false;
...
CFileLoader::LoadCollisionModel(uint8 *buf, CColModel &model, char *modelname)
{
	...
	model.boundingSphere.radius = ReadCollisionFloat(buf);
	model.boundingSphere.center = ReadCollisionVector(buf+4);
	model.boundingBox.min = ReadCollisionVector(buf+16);
	model.boundingBox.max = ReadCollisionVector(buf+28);
	model.numSpheres = ReadCollisionUInt16(buf+40);
	buf += 44;
	if(model.numSpheres > 0){
		...
		for(i = 0; i < model.numSpheres; i++){
			model.spheres[i].Set(ReadCollisionFloat(buf), ReadCollisionVector(buf+4), buf[16], buf[17]);
			buf += 20;
		}
	}else
		model.spheres = nil;

	model.numLines = ReadCollisionUInt16(buf);
	buf += 4;
	if(model.numLines > 0){
	...
	model.numBoxes = ReadCollisionUInt16(buf);
	buf += 4;
	if(model.numBoxes > 0){
		model.boxes = (CColBox*)RwMalloc(model.numBoxes*sizeof(CColBox));
		REGISTER_MEMPTR(&model.boxes);
		for(i = 0; i < model.numBoxes; i++){
			model.boxes[i].Set(ReadCollisionVector(buf), ReadCollisionVector(buf+12), buf[24], buf[25]);
			buf += 28;
		}
	}else
		model.boxes = nil;

	int32 numVertices = ReadCollisionUInt16(buf);
	buf += 4;
	if(numVertices > 0){
		model.vertices = (CompressedVector*)RwMalloc(numVertices*sizeof(CompressedVector));
		REGISTER_MEMPTR(&model.vertices);
		for(i = 0; i < numVertices; i++){
			model.vertices[i].Set(ReadCollisionFloat(buf), ReadCollisionFloat(buf+4), ReadCollisionFloat(buf+8));
#if 0
	...
	model.numTriangles = ReadCollisionUInt16(buf);
	buf += 4;
	if(model.numTriangles > 0){
		model.triangles = (CColTriangle*)RwMalloc(model.numTriangles*sizeof(CColTriangle));
		REGISTER_MEMPTR(&model.triangles);
		for(i = 0; i < model.numTriangles; i++){
			model.triangles[i].Set(ReadCollisionUInt32(buf), ReadCollisionUInt32(buf+4),
			                       ReadCollisionUInt32(buf+8), buf[12]);
			buf += 16;
		}
	}else
		model.triangles = nil;
...
CFileLoader::FinishLoadClumpFile(RwStream *stream, uint32 id)
{
	RpClump *clump;
	CClumpModelInfo *mi;
	const char *modelName = CModelInfo::GetModelInfo(id)->GetModelName();
	...
	if(clump){
#ifdef NINTENDO_WII
		wiiLog("WII clump finalize: model=%u name=%s read complete clump=%p\n", id, modelName, clump);
		wiiLog("WII clump finalize: model=%u InitClump begin\n", id);
#endif
		InitClump(clump);
#ifdef NINTENDO_WII
		wiiLog("WII clump finalize: model=%u InitClump complete\n", id);
#endif
		mi = (CClumpModelInfo*)CModelInfo::GetModelInfo(id);
#ifdef NINTENDO_WII
		wiiLog("WII clump finalize: model=%u SetClump begin info=%p\n", id, mi);
#endif
		mi->SetClump(clump);
#ifdef NINTENDO_WII
		wiiLog("WII clump finalize: model=%u SetClump complete object=%p\n", id, mi->GetRwObject());
#endif
		return true;
	}else{
		printf("FAILED\n");
...
CFileLoader::Load2dEffect(const char *line)
{
	...
	int32 &effectIndex = CModelInfo::Get2dEffectStore().allocPtr;
	effect = CModelInfo::Get2dEffectStore().Alloc();
	if(mi == nil || effect == nil){
#ifdef NINTENDO_WII
		wiiLog("WII 2DFX: rejected model=%d modelInfo=%p allocation=%p index=%d\n",
		       id, mi, effect, CModelInfo::Get2dEffectStore().allocPtr);
#endif
