...
#ifdef NINTENDO_WII
	if(wiiIdleFrame <= 8)
		wiiLog("WII render: frame=%u menu=%s fade=%d camera=%p world=%p\n",
		       wiiIdleFrame, FrontEndMenuManager.m_bMenuActive ? "yes" : "no",
		       TheCamera.GetScreenFadeStatus(), Scene.camera, Scene.world);
#endif
...
#ifdef NINTENDO_WII
		if(wiiIdleFrame <= 8)
			wiiLog("WII render: frame=%u branch=world construct begin\n", wiiIdleFrame);
#endif
...
#ifdef NINTENDO_WII
		if(wiiIdleFrame <= 8)
			wiiLog("WII render: frame=%u construct complete\n", wiiIdleFrame);
#endif
...
#ifdef NINTENDO_WII
		// Splits "the game asked for a white backdrop" from "the backend drew
		// one".  Everything here decides what covers the whole screen.
		if(wiiIdleFrame <= 8 || wiiIdleFrame % 300 == 0)
			wiiLog("WII sky: frame=%u lightning=%d dark=%d top=%d,%d,%d bottom=%d,%d,%d fade=%d brightness=%d\n",
			       wiiIdleFrame, CWeather::LightningFlash ? 1 : 0,
			       CCullZones::CamNoRain() ? 1 : 0,
			       (int)CTimeCycle::GetSkyTopRed(), (int)CTimeCycle::GetSkyTopGreen(),
			       (int)CTimeCycle::GetSkyTopBlue(), (int)CTimeCycle::GetSkyBottomRed(),
			       (int)CTimeCycle::GetSkyBottomGreen(), (int)CTimeCycle::GetSkyBottomBlue(),
			       (int)CDraw::FadeValue, (int)FrontEndMenuManager.m_PrefsBrightness);
#endif
...
#ifdef NINTENDO_WII
		if(wiiIdleFrame <= 8)
			wiiLog("WII render: frame=%u camera update begun\n", wiiIdleFrame);
#endif
...
#ifdef NINTENDO_WII
		if(wiiIdleFrame <= 8)
			wiiLog("WII render: frame=%u scene complete\n", wiiIdleFrame);
#endif
...
#ifdef NINTENDO_WII
		if(wiiIdleFrame <= 8)
			wiiLog("WII render: frame=%u branch=menu-or-fade\n", wiiIdleFrame);
#endif
		CDraw::CalculateAspectRatio();
