...
{
#if defined NINTENDO_WII
	// First Wii input of the frame, so this is where both stacks are sampled;
	// CapturePad below reads whatever this leaves behind.  See WiiPad.h for why
	// the scan cannot live inside either consumer.
	WiiPadScan();
	WiiPadUpdateRumble();
...
#ifdef NINTENDO_WII
// The two branches in the look functions below disagree with each other in two
// ways that a stick held off axis exposes immediately.
//
// They start counting at different places -- 10 on the horizontal against 40 on
// the vertical -- so a stick pushed diagonally turns the camera about seven
// times further sideways than up, which Cam.cpp's 0.6 pitch factor then widens
// again.  And the steep branch replaces the gentle one instead of continuing it,
// so the output collapses as an axis crosses 85: on the horizontal 84 yields 146
// and 86 yields 4, and pushing further turns the camera slower until the axis
// reaches about 122.  On a diagonal the two axes cross 85 at different amounts of
// stick travel, which is what leaves the directions feeling unrelated to one
// another.
//
// One monotonic curve replaces both, identical on both axes.  Its shape is the
// one this port already uses for the Wiimote pointer: a linear term blended with
// a quadratic one, where the quadratic half keeps small deflections slow enough
// to aim with and the linear half stops the bottom of the travel from doing
// nothing perceptible.  The peak is the 170 the original reached at full
// deflection, so the fastest the camera can turn is unchanged -- and because the
// curve saturates there, a stick sensitivity above 1.0 buys a shorter throw to
// reach that speed rather than a higher one.
static const float WiiLookDeadzone = 10.0f;
static const float WiiLookFullScale = 128.0f;
static const float WiiLookLinear = 0.35f;
static const float WiiLookPeak = (127.0f / 32.0f) * (128.0f - 85.0f); // 170.66f

static int16
WiiLookAxis(float axis)
{
	float t = (Abs(axis) - WiiLookDeadzone) / (WiiLookFullScale - WiiLookDeadzone);
	if ( t <= 0.0f )
		return 0;
	if ( t > 1.0f )
		t = 1.0f;

	const float out = t * (WiiLookLinear + (1.0f - WiiLookLinear) * t) * WiiLookPeak;
	return (int16) ( axis < 0.0f ? -out : out );
}
#endif
...
#ifdef NINTENDO_WII
	// Neither of the conditions the branches below carry survives the rewrite,
	// and neither is a loss here.  Using3rdPersonMouseCam only chose between the
	// two branches, and GetLookBehindForPed reads R3, which src/wii-port/
	// WiiPad.cpp binds on no supported controller.
	return WiiLookAxis(axis);
#else
...
#ifdef NINTENDO_WII
	// Same curve as the horizontal, which is the point: this is where the two
	// used to part company at 40 against 10.  Both inversions above still apply,
	// because they decide which way the axis points rather than how far it goes.
	return WiiLookAxis((float)axis);
#else
	if ( Abs(axis) > 85 && !GetLookBehindForPed() )
		return (int16) ( (axis + ( ( axis > 0 ) ? -85 : 85) )
							* (127.0f / 32.0f) ); // 3.96875f
