...
bool CGame::Initialise(const char* datFile)
{
...
#ifdef NINTENDO_WII
	wiiLog("WII level init: generic TXD\n");
#endif
...
#ifdef NINTENDO_WII
	wiiLog("WII level init: particle TXD\n");
#endif
	...
	CRestart::Initialise();
#ifdef NINTENDO_WII
	wiiLog("WII level init: core world services\n");
#endif
	...
	POP_MEMID();
#ifdef NINTENDO_WII
	wiiLog("WII level init: world and animation managers\n");
#endif
...
#ifdef NINTENDO_WII
	wiiLog("WII level init: GTA3.IMG registered\n");
#endif

	CFileLoader::LoadLevel("DATA\\DEFAULT.DAT");
#ifdef NINTENDO_WII
	wiiLog("WII level init: DEFAULT.DAT\n");
#endif
	CFileLoader::LoadLevel(datFile);
#ifdef NINTENDO_WII
	wiiLog("WII level init: GTA_VC.DAT\n");
#endif
...
#ifdef NINTENDO_WII
	wiiLog("WII level init: paths and water\n");
#endif
	TheConsole.Init();
	CDraw::SetFOV(120.0f);
...
#ifdef NINTENDO_WII
	CBaseModelInfo *maleModel = CModelInfo::GetModelInfo(MI_MALE01);
	int32 maleTxd = maleModel ? maleModel->GetTxdSlot() : -1;
	if(maleModel && maleTxd >= 0 && !CStreaming::HasModelLoaded(MI_MALE01)){
		CStreamingInfo &maleInfo = CStreaming::ms_aInfoForModel[MI_MALE01];
		CStreamingInfo &maleTxdInfo = CStreaming::ms_aInfoForModel[STREAM_OFFSET_TXD + maleTxd];
		wiiLog("WII level init: male01 directory offset=%u size=%u links=%p/%p\n",
		       maleInfo.m_position, maleInfo.m_size, maleInfo.m_prev, maleInfo.m_next);
		wiiLog("WII level init: male01 TXD directory offset=%u size=%u links=%p/%p\n",
		       maleTxdInfo.m_position, maleTxdInfo.m_size,
		       maleTxdInfo.m_prev, maleTxdInfo.m_next);
		wiiLog("WII level init: loading male01 dependencies modelState=%d txd=%d txdState=%d\n",
		       CStreaming::ms_aInfoForModel[MI_MALE01].m_loadState, maleTxd,
		       CStreaming::ms_aInfoForModel[STREAM_OFFSET_TXD + maleTxd].m_loadState);
		CStreaming::RemoveTxd(maleTxd);
		CStreaming::RequestTxd(maleTxd,
		                       STREAMFLAGS_DONT_REMOVE | STREAMFLAGS_PRIORITY);
		CStreaming::LoadModelSynchronously(STREAM_OFFSET_TXD + maleTxd);
		wiiLog("WII level init: male01 TXD dependency loaded=%s state=%d\n",
		       CStreaming::HasTxdLoaded(maleTxd) ? "yes" : "no",
		       CStreaming::ms_aInfoForModel[STREAM_OFFSET_TXD + maleTxd].m_loadState);
		CStreaming::RemoveModel(MI_MALE01);
		CStreaming::RequestModel(MI_MALE01,
		                         STREAMFLAGS_DONT_REMOVE | STREAMFLAGS_PRIORITY);
		CStreaming::LoadModelSynchronously(MI_MALE01);
	}
#endif
#ifdef NINTENDO_WII
	wiiLog("WII level init: initial streaming male01 loaded=%s object=%p txd=%d anim=%d\n",
	       CStreaming::HasModelLoaded(MI_MALE01) ? "yes" : "no",
	       maleModel ? maleModel->GetRwObject() : nil,
	       maleModel ? maleModel->GetTxdSlot() : -1,
	       maleModel ? maleModel->GetAnimFileIndex() : -1);
	wiiLog("WII level init: island streaming cleanup begin\n");
#endif
	CStreaming::RemoveIslandsNotUsed(currLevel);
#ifdef NINTENDO_WII
	wiiLog("WII level init: island streaming cleanup complete\n");
#endif
	printf("Streaming uses %zuK of its memory", CStreaming::ms_memoryUsed / 1024); // original modifier was %d

	LoadingScreen("Loading the Game", "Load animations", GetRandomSplashScreen());
#ifdef NINTENDO_WII
	wiiLog("WII level init: animation files begin\n");
#endif
	PUSH_MEMID(MEMID_ANIMATION);
	CAnimManager::LoadAnimFiles();
	POP_MEMID();
#ifdef NINTENDO_WII
	wiiLog("WII level init: animation files\n");
#endif
	...
	POP_MEMID();
#ifdef NINTENDO_WII
	wiiLog("WII level init: scripts\n");
#endif

	LoadingScreen("Loading the Game", "Setup game variables", nil);
#ifdef NINTENDO_WII
	wiiLog("WII level init: world systems begin\n");
#endif
	...
	CGarages::Init();
#ifdef NINTENDO_WII
	wiiLog("WII level init: world systems complete\n");
#endif
...
#ifdef NINTENDO_WII
	wiiLog("WII level init: transport systems begin\n");
#endif
	CTrain::InitTrains();
	CPlane::InitPlanes();
	CCredits::Init();
	...
	CReplay::Init();
#ifdef NINTENDO_WII
	wiiLog("WII level init: transport systems complete\n");
#endif
	...
	{
#ifdef NINTENDO_WII
		wiiLog("WII level init: test script start begin\n");
#endif
		CTheScripts::StartTestScript();
#ifdef NINTENDO_WII
		wiiLog("WII level init: test script start complete\n");
		wiiLog("WII level init: initial script process begin\n");
#endif
		CTheScripts::Process();
#ifdef NINTENDO_WII
		wiiLog("WII level init: initial camera process begin\n");
#endif
		TheCamera.Process();
#ifdef NINTENDO_WII
		wiiLog("WII level init: initial script and camera complete\n");
#endif
	}
...
#ifdef NINTENDO_WII
	wiiLog("WII level init: final audio state complete\n");
#endif
	return true;
}
...
#ifdef NINTENDO_WII
	if(traceWiiProcess)
		wiiLog("WII frame: streaming complete frame=%u\n", wiiProcessCount);
#endif
...
#ifdef NINTENDO_WII
		if(traceWiiProcess)
			wiiLog("WII frame: scripts complete frame=%u\n", wiiProcessCount);
#endif
...
#ifdef NINTENDO_WII
		if(traceWiiProcess)
			wiiLog("WII frame: world complete frame=%u\n", wiiProcessCount);
#endif
