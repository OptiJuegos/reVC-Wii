...
static int32
ReadIfpInt32(const void *source)
{
	uint32 value;
...
static void
ConvertIfpFloatBuffer(float *values, int32 count)
{
#ifdef BIGENDIAN
	for(int32 i = 0; i < count; i++)
		values[i] = BSWAP_F32(values[i]);
#else
	(void)values;
	(void)count;
#endif
...
#ifdef NINTENDO_WII
		if(i == 0)
			wiiLog("WII animation: creating associations with model=%d object=%p\n",
			       ms_aAnimAssocDefinitions[i].modelIndex,
			       CModelInfo::GetModelInfo(ms_aAnimAssocDefinitions[i].modelIndex)->GetRwObject());
#endif

		CBaseModelInfo *mi = CModelInfo::GetModelInfo(ms_aAnimAssocDefinitions[i].modelIndex);
		if(mi == nil){
#ifdef NINTENDO_WII
			wiiLog("WII animation: association group=%d has no model info\n", i);
#endif
			continue;
		}
...
#ifdef NINTENDO_WII
			if(i == 0)
				wiiLog("WII animation: base clump unavailable; associations deferred\n");
#endif
			continue;
		}
...
CAnimManager::LoadAnimFile(const char *filename)
{
	RwStream *stream;
#ifdef NINTENDO_WII
	wiiLog("WII animation: opening %s\n", filename);
#endif
	stream = RwStreamOpen(rwSTREAMFILENAME, rwSTREAMREAD, filename);
#ifdef NINTENDO_WII
	wiiLog("WII animation: open %s stream=%p\n", filename, stream);
#endif
	assert(stream);
	if(stream == nil)
		return;
...
CAnimManager::LoadAnimFile(RwStream *stream, bool compress, char (*uncompressedAnims)[32])
{
	...
	auto ReadIfpHeader = [stream](IfpHeader *header){
		RwStreamRead(stream, header, sizeof(IfpHeader));
		header->size = BSWAP_U32(header->size);
	};
	...
	ROUNDSIZE(info.size);
#ifdef NINTENDO_WII
	wiiLog("WII animation: root %.4s size=%u info=%.4s size=%u\n",
	       anpk.ident, anpk.size, info.ident, info.size);
#endif
	assert(info.size <= sizeof(buf));
	...
	if(animBlock){
		if(animBlock->numAnims == 0){
			animBlock->numAnims = ReadIfpInt32(buf);
...
#ifdef NINTENDO_WII
	wiiLog("WII animation: block=%s animations=%d\n",
	       animBlock->name, animBlock->numAnims);
#endif
	...
	for(j = 0; j < animBlock->numAnims; j++){
#ifdef NINTENDO_WII
		if((j % 25) == 0 || animBlock->numAnims <= 25)
			wiiLog("WII animation: reading animation %d/%d\n", j, animBlock->numAnims);
#endif
		...
		if(info.size > sizeof(buf))
			return;
		RwStreamRead(stream, buf, info.size);
		hier->numSequences = ReadIfpInt32(buf);
#ifdef NINTENDO_WII
		wiiLog("WII animation: hierarchy=%s index=%d sequences=%d\n",
		       hier->name, j, hier->numSequences);
		if(hier->numSequences < 0 || hier->numSequences > 256){
			wiiLog("WII animation: invalid sequence count block=%s hierarchy=%s count=%d\n",
			       animBlock->name, hier->name, hier->numSequences);
			return;
		}
#endif
		...
		for(k = 0; k < hier->numSequences; k++, seq++){
			...
			ReadIfpHeader(&anim);
			ROUNDSIZE(anim.size);
			assert(anim.size <= sizeof(buf));
			if(anim.size > sizeof(buf))
				return;
			RwStreamRead(stream, buf, anim.size);
			int numFrames = ReadIfpInt32(buf + 28);
#ifdef NINTENDO_WII
			if(numFrames < 0 || numFrames > 10000){
				wiiLog("WII animation: invalid frame count block=%s hierarchy=%s sequence=%d count=%d\n",
				       animBlock->name, hier->name, k, numFrames);
				return;
			}
#endif
			seq->SetName(buf);
			if(anim.size == 44)
				seq->SetBoneTag(ReadIfpInt32(buf + 40));
			if(numFrames == 0)
				continue;
...
#ifdef NINTENDO_WII
	wiiLog("WII animation: block=%s load complete animations=%d\n",
	       animBlock->name, animBlock->numAnims);
#endif
}
