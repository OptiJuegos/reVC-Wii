...
#ifdef NINTENDO_WII
			CBaseModelInfo *playerModel = CModelInfo::GetModelInfo(MI_PLAYER);
			int32 playerTxd = playerModel->GetTxdSlot();
			wiiLog("WII scripts: loading player model txd=%d txdLoaded=%s\n",
			       playerTxd, CStreaming::HasTxdLoaded(playerTxd) ? "yes" : "no");
			if(!CStreaming::HasTxdLoaded(playerTxd))
				CStreaming::LoadModelSynchronously(STREAM_OFFSET_TXD + playerTxd);
			if(CStreaming::HasTxdLoaded(playerTxd))
				CStreaming::LoadModelSynchronously(MI_PLAYER);
			wiiLog("WII scripts: player dependencies modelLoaded=%s object=%p txdLoaded=%s\n",
			       CStreaming::HasModelLoaded(MI_PLAYER) ? "yes" : "no",
			       playerModel->GetRwObject(),
			       CStreaming::HasTxdLoaded(playerTxd) ? "yes" : "no");
#else
			CStreaming::LoadAllRequestedModels(false);
#endif
		}
#ifdef NINTENDO_WII
		if(!CStreaming::HasModelLoaded(MI_PLAYER) || CModelInfo::GetModelInfo(MI_PLAYER)->GetRwObject() == nil){
			wiiLog("WII scripts: player creation deferred because its model is unavailable\n");
			m_nIp = createPlayerIp;
			m_nWakeTime = CTimer::GetTimeInMilliseconds() + 1;
			return 1;
		}
#endif
