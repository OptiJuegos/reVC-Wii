...
#ifdef LIBRW
namespace rw { namespace gx { void setNativeTextureTrace(bool32 enabled); } }
#endif
#endif
...
#ifdef NINTENDO_WII
static bool wiiTraceTxdRead;

void
SetWiiTxdReadTrace(bool enabled)
{
	wiiTraceTxdRead = enabled;
}
#endif
...
RwTextureGtaStreamRead(RwStream *stream)
{
	...
	RwTexture *tex;
#ifdef NINTENDO_WII
	int32 chunkSearchPosition = STREAMPOS(stream);
	if(wiiTraceTxdRead)
		wiiLog("WII TXD trace: native chunk search position=%d\n", chunkSearchPosition);
#endif
...
#ifdef NINTENDO_WII
	if(wiiTraceTxdRead)
		wiiLog("WII TXD trace: native chunk found payload=%u version=0x%x position=%d\n",
		       size, version, STREAMPOS(stream));
#endif
#ifdef GTA_PC
...
#ifdef NINTENDO_WII
	if(wiiTraceTxdRead)
		wiiLog("WII TXD trace: native read complete position=%d expected-end=%d\n",
		       STREAMPOS(stream), chunkSearchPosition + 12 + (int32)size);
#endif
#ifdef GTA_PC
...
RwTexDictionaryGtaStreamRead(RwStream *stream)
{
	...
	if(RwStreamRead(stream, &numTextures, size) != size)
		return nil;
	numTextures = (RwInt32)BSWAP_I32(numTextures);
#ifdef NINTENDO_WII
	if(wiiTraceTxdRead)
		wiiLog("WII TXD trace: dictionary textures=%d structSize=%u version=0x%x\n",
		       numTextures, size, version);
#endif
	...
	while(numTextures--){
#ifdef NINTENDO_WII
		bool traceNativeTexture = wiiTraceTxdRead && textureIndex >= 12;
		if(traceNativeTexture)
			wiiLog("WII TXD trace: texture index=%d begin\n", textureIndex);
#ifdef LIBRW
		rw::gx::setNativeTextureTrace(traceNativeTexture);
#endif
#endif
		tex = RwTextureGtaStreamRead(stream);
#if defined(NINTENDO_WII) && defined(LIBRW)
		rw::gx::setNativeTextureTrace(false);
#endif
		if(tex == nil){
...
#ifdef NINTENDO_WII
		if(wiiTraceTxdRead && textureIndex >= 13)
			wiiLog("WII TXD trace: texture index=%d complete\n", textureIndex);
#endif
		textureIndex++;
	}
...
#ifdef NINTENDO_WII
	if(wiiTraceTxdRead)
		wiiLog("WII TXD trace: dictionary complete textures=%d position=%d\n",
		       textureIndex, STREAMPOS(stream));
#endif
