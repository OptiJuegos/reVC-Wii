...
static uint64
readDxt48(const uint8 *source)
{
	// DXT5 stores sixteen three-bit alpha indices in exactly six bytes.
	// Assemble the little-endian field explicitly so no adjacent color bytes
	// are included in the endian conversion.
	uint64 value = 0;
	for(int32 i = 0; i < 6; i++)
		value |= (uint64)source[i] << (i*8);
	return value;
}

static void
writeDxt48(uint8 *destination, uint64 value)
{
	for(int32 i = 0; i < 6; i++){
...
decompressDXT1(uint8 *adst, int32 w, int32 h, uint8 *src)
{
	...
	for(int32 j = 0; j < w*h/2; j += 8){
		/* calculate colors */
		uint32 col0 = readDxt16(&src[j+0]);
		uint32 col1 = readDxt16(&src[j+2]);
		...
		/* make index list */
		uint32 indices = readDxt32(&src[j+4]);
...
decompressDXT3(uint8 *adst, int32 w, int32 h, uint8 *src)
{
	...
	for(int32 j = 0; j < w*h; j += 16){
		/* calculate colors */
		uint32 col0 = readDxt16(&src[j+8]);
		uint32 col1 = readDxt16(&src[j+10]);
		c[0][0] = ((col0>>11) & 0x1F)*0xFF/0x1F;
		c[0][1] = ((col0>> 5) & 0x3F)*0xFF/0x3F;
		c[0][2] = ( col0      & 0x1F)*0xFF/0x1F;
		...
		/* make index list */
		uint32 indices = readDxt32(&src[j+12]);
		for(int32 k = 0; k < 16; k++){
			idx[k] = indices & 0x3;
			indices >>= 2;
		}
		uint64 alphas = readDxt64(&src[j+0]);
		for(int32 k = 0; k < 16; k++){
			a[k] = (alphas & 0xF)*17;
			alphas >>= 4;
		}
...
decompressDXT5(uint8 *adst, int32 w, int32 h, uint8 *src)
{
	...
	for(int32 j = 0; j < w*h; j += 16){
		...
		// only 6 indices
		uint64 alphas = readDxt48(&src[j+2]);
		for(int32 k = 0; k < 16; k++){
			aidx[k] = alphas & 0x7;
			alphas >>= 3;
		}
