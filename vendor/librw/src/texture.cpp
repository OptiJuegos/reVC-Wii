...
{
#ifdef RW_GX
	if(gx::nativeTextureTraceEnabled())
		SYS_Report("WII GX native: dispatcher find struct begin\n");
#endif
	if(!findChunk(stream, ID_STRUCT, nil, nil)){
		RWERROR((ERR_CHUNK, "STRUCT"));
		return nil;
...
#ifdef RW_GX
	if(gx::nativeTextureTraceEnabled())
		SYS_Report("WII GX native: dispatcher platform=%u rewind begin\n", platform);
#endif
	stream->seek(-16);
#ifdef RW_GX
	if(gx::nativeTextureTraceEnabled())
		SYS_Report("WII GX native: dispatcher rewind complete\n");
#endif
#ifdef RW_GX
	if(platform == PLATFORM_D3D8)
		return gx::readNativeTextureD3D8(stream);
#endif
